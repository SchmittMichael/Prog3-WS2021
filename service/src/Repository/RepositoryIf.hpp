...
class RepositoryIf {
  public:
    ...
    virtual std::optional<ReminderApp::Core::Model::Reminder> postReminder(int listId, std::string title, int position, time_t date) = 0;
    virtual std::optional<ReminderApp::Core::Model::Reminder> putReminder(int listId, int itemId, std::string title, int position, time_t date) = 0;
